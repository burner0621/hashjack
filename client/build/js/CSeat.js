function CSeat() {
    ...
    this.increaseCredit = function (iCreditToAdd) {
        _iCredit = parseFloat(_iCredit) + iCreditToAdd;
    };
    ...
    this.bet = function (iBet, bSplit) {
        if (bSplit) {
            _oCurBetText.text = (iBet / 2) + TEXT_CURRENCY;
            _oCurSplitBetText.text = (iBet / 2) + TEXT_CURRENCY;
        } else {
            _oCurBetText.text = iBet + TEXT_CURRENCY;
        }
    };
    ...
    this.decreaseCredit = function (iCreditToSubtract) {
        _iCredit = (parseFloat(_iCredit) - iCreditToSubtract).toFixed(3);
    };
    ...
    this.showWinner = function (iHand, szWinner, iTotalWin, roundFee) {
        if (iTotalWin > 0) {
            if (iHand === 0) {
        ...
        document.getElementById("endRound").value = _iCredit
        if (szWinner === TEXT_SHOW_WIN_PLAYER)
            document.getElementById("endRound").setAttribute("winflag",2)
        else if (szWinner === TEXT_SHOW_STANDOFF)
            document.getElementById("endRound").setAttribute("winflag",1)
        else
            document.getElementById("endRound").setAttribute("winflag",0)
        document.getElementById("endRound").setAttribute("earning",iTotalWin)
        document.getElementById("endRound").setAttribute("roundfee",roundFee)
